...
// (No nested backticks anywhere to avoid SyntaxError.)
...
+ '<style>'
+ ':root{--bg:#0b1220;--panel:#0f172a;--muted:#94a3b8;--text:#e2e8f0;--brand:#f59e0b;--accent:#1f2937;--ok:#16a34a;--warn:#f59e0b;--bad:#dc2626}'
+ '*{box-sizing:border-box}body{font-family:system-ui,Arial;background:var(--bg);color:var(--text);margin:0}'
+ '.wrap{max-width:1200px;margin:0 auto;padding:16px}.panel{background:var(--panel);border:1px solid #1f2937;border-radius:12px;padding:12px;margin:12px 0}'
+ '.grid{display:grid;gap:12px}.grid.cols-4{grid-template-columns:repeat(4,minmax(0,1fr))}'
...
+ '.btn{background:var(--brand);color:#111;border:none;cursor:pointer;font-weight:700}.muted{color:var(--muted)}'
+ 'table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #223048;padding:8px;text-align:left;font-size:14px}'
+ '.tabs{display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap}.tab{padding:8px 12px;border-radius:9999px;border:1px solid #334155;background:#0b1528;cursor:pointer}'
+ '.tab.active{background:#ef4444;border-color:#ef4444}.badge{padding:4px 8px;border-radius:9999px;background:#1f2937}.tiny{font-size:12px}'
+ '</style></head><body>'
+ '<div class="wrap">'
...
+ '</div>'
+ '<div class="panel"><div class="muted tiny">Limits</div><div>Min 20 - Max 1000 • Max Payout 20000</div></div>'
+ '<div class="panel">'
...
+ '<strong>Recent Tickets</strong><button class="btn" onclick="loadTickets()">Refresh</button>'
+ '</div>'
+ '<table id="t"><thead><tr>'
...
+ '</div>'
+ '<script>'
+ "var currentProduct='FOOTBALL';"
+ "Array.prototype.forEach.call(document.querySelectorAll('#tabs .tab'),function(btn){"
+ " btn.addEventListener('click',function(){"
+ "  Array.prototype.forEach.call(document.querySelectorAll('#tabs .tab'),function(b){b.classList.remove('active');});"
+ "  btn.classList.add('active'); currentProduct=btn.dataset.product; });});"
+ "function fmtKES(c){return 'KES '+Math.round((c||0)/100)}"
+ "async function placeBet(){"
+ " var key=document.getElementById('key').value.trim();"
+ " var stakeKES=parseInt(document.getElementById('stake').value||'0',10);"
+ " var stake_cents=stakeKES*100;"
+ " var odds=parseFloat(document.getElementById('odds').value||'2.00');"
+ " var res=await fetch('/bets/place',{method:'POST',headers:{'Content-Type':'application/json','x-cashier-key':key},body:JSON.stringify({stake_cents:stake_cents,odds:odds,product:currentProduct})});"
+ " var data=await res.json(); alert(JSON.stringify(data,null,2)); loadTickets();"
+ "}"
+ "async function loadTickets(){"
+ " var key=document.getElementById('key').value.trim();"
+ " var res=await fetch('/cashier/tickets',{headers:{'x-cashier-key':key}});"
+ " var rows=await res.json(); var tb=document.querySelector('#t tbody'); tb.innerHTML='';"
...
+ "   '<td>'+r.status+'</td>' +"
+ "   '<td><a href=\"/tickets/'+\"'+\"'+\"' + r.uid + '\" target=\"_blank\">Print</a></td>'; "
+ "  tb.appendChild(tr);"
+ " });"
+ "}"
+ '</script>'
